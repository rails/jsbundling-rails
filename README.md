...
Use [Bun](https://bun.sh), [esbuild](https://esbuild.github.io), [rolldown](https://rolldown.rs), [rollup.js](https://rollupjs.org), or [Webpack](https://webpack.js.org) to bundle your JavaScript, then deliver it via the asset pipeline in Rails. This gem provides installers to get you going with the bundler of your choice in a new Rails application, and a convention to use `app/assets/builds` to hold your bundled output as artifacts that are not checked into source control (the installer adds this directory to `.gitignore` by default).
...
You can configure your bundler options in the `build` script in `package.json` or via the installer-generated `bun.config.js` for Bun, `rolldown.config.mjs` for rolldown, `rollup.config.js` for rollup.js or `webpack.config.json` for Webpack (esbuild does not have a default configuration format, and we don't intend to use esbuild as an API in order to hack around it).
...
## Installation
If you are installing esbuild, rolldown, rollup, or webpack, you must already have node installed on your system. You will also need npx version 7.1.0 or later.
...
```
./bin/rails javascript:install:[bun|esbuild|rolldown|rollup|webpack]
```

Or, in Rails 7+, you can preconfigure your new application to use a specific bundler with `rails new myapp -j [bun|esbuild|rolldown|rollup|webpack]`.
